...
export default function TelegramModal({ vacancy, onClose, onSent }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="flex w-full max-w-lg flex-col gap-4 rounded-xl bg-gray-900 p-6 shadow-2xl">
        <div className="flex items-start justify-between gap-2">
          <div>
            <h2 className="text-lg font-semibold text-white">Отправить в Telegram</h2>
            <p className="mt-0.5 text-sm text-gray-400">
              Получатель: <span className="font-medium text-indigo-400">{vacancy.contact_value}</span>
            </p>
          </div>
        ...
        <textarea
          ...
          rows={12}
          className="w-full resize-y rounded-lg bg-gray-800 px-3 py-2 text-sm text-gray-100 placeholder-gray-500 outline-none ring-1 ring-gray-700 focus:ring-indigo-500"

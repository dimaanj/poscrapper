...
const STATUS_BADGE: Record<VacancyStatus, { label: string; cls: string }> = {
  new: { label: 'Новая', cls: 'bg-blue-900/60 text-blue-300' },
  saved: { label: 'Сохранена', cls: 'bg-yellow-900/60 text-yellow-300' },
  applied: { label: 'Отправлено', cls: 'bg-green-900/60 text-green-300' },
  skipped: { label: 'Пропущена', cls: 'bg-gray-800 text-gray-500' },
};
      ...
      >
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-2 min-w-0">
            <span className="text-xs font-semibold text-indigo-400 shrink-0">{vacancy.channel}</span>
            <span className="text-xs text-gray-600">·</span>
            <span className="text-xs text-gray-500 shrink-0">{formatDate(vacancy.matched_at)}</span>
          </div>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          {/* Contact badge */}
          <span className="flex items-center gap-1 rounded-lg bg-gray-800 px-2 py-1 text-xs text-gray-300">
          ...
          {vacancy.tg_link && (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="rounded-lg bg-gray-800 px-2 py-1 text-xs text-indigo-400 hover:text-indigo-300 hover:bg-gray-700"
            >
              ↗ Открыть пост
            </a>
          )}
          ...
          <div className="ml-auto flex flex-wrap gap-2">
            ...
            {vacancy.status === 'new' && (
              <>
                <button
                  ...
                  className="rounded-lg bg-gray-800 px-3 py-1 text-xs font-medium text-yellow-400 hover:bg-gray-700 disabled:opacity-50"
                >
                  ★ Сохранить
                </button>
                <button
                  ...
                  className="rounded-lg bg-gray-800 px-3 py-1 text-xs font-medium text-gray-500 hover:bg-gray-700 disabled:opacity-50"
                >
                  Skip
                </button>
              </>
            ...
            {vacancy.status === 'skipped' && (
              <button
                ...
                className="rounded-lg bg-gray-800 px-3 py-1 text-xs font-medium text-gray-400 hover:bg-gray-700 disabled:opacity-50"
              >
                ↩ Вернуть
              </button>
            )}
